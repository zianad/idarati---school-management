...
const DeveloperLogo = () => (
    <svg width="200" height="200" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 297.31 290">
        <defs>
            <style>
            {`
                .cls-1-logo{fill:none;}
                .cls-2-logo{font-size:22px;font-family:Damavand-Regular, sans-serif;font-weight:300;text-anchor:middle;}
                ...
                .cls-9-logo{font-size:38px;font-family:Damavand-Regular, sans-serif;text-anchor:middle;}
            `}
            </style>
        ...
        <g transform="scale(1.2) translate(-29, -30)">
        ...
        <text className="cls-9-logo" x="148.655" y="225">idarati</text>
        <text className="cls-2-logo" x="148.655" y="260">0622119357</text>
    </svg>
);
...
const LoginPage: React.FC = () => {
  ...
  const { login, schools } = useAppContext();
  const { t, dir } = useLanguage();

  const [isForgotModalOpen, setIsForgotModalOpen] = useState(false);
  const [superAdminCode, setSuperAdminCode] = useState('');
  const [superAdminError, setSuperAdminError] = useState('');
  const [retrievedSchools, setRetrievedSchools] = useState<School[] | null>(null);
  ...
  const handleCloseForgotModal = () => {
      ...
      setSuperAdminCode('');
      setSuperAdminError('');
      setRetrievedSchools(null);
  };
  
  const handleSuperAdminSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (superAdminCode === 'Abzn11241984') {
          setRetrievedSchools(schools);
          setSuperAdminError('');
      } else {
          setSuperAdminError(t('invalidCode'));
          setRetrievedSchools(null);
      }
  };
  
  const inputClass = "w-full px-4 py-3 text-lg text-gray-900 bg-gray-100 border-2 border-gray-300 rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500";
  const buttonClass = "w-full px-4 py-3 font-bold text-lg text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 shadow-lg hover:shadow-xl transition-all duration-200";

  return (
    <>
      <div dir={dir} className="min-h-screen flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
        <div className="w-full max-w-md p-10 space-y-6 bg-white rounded-2xl shadow-2xl dark:bg-gray-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="secret-code" className="sr-only">{t('secretCode')}</label>
              <input
                id="secret-code"
                type="password"
                value={code}
           ...
           <div className="text-center">
                <button onClick={() => setIsForgotModalOpen(true)} className="text-sm font-medium text-blue-600 hover:underline dark:text-blue-400">
                    {t('forgotCode')}
                </button>
            </div>
        </div>
      </div>
      <Modal isOpen={isForgotModalOpen} onClose={handleCloseForgotModal} title={t('remindCodes')}>
          {!retrievedSchools ? (
            <form onSubmit={handleSuperAdminSubmit} className="space-y-4">
                <p className="text-gray-600 dark:text-gray-400">{t('enterSuperAdminCode')}</p>
                <div>
                    <label htmlFor="super-admin-code" className="sr-only">{t('secretCode')}</label>
                    <input
                        id="super-admin-code"
                        type="password"
                ...
                {superAdminError && <p className="text-red-500">{superAdminError}</p>}
                <div className="flex justify-end">
                     <button type="submit" className={buttonClass.replace('w-full', '')}>
                        {t('showCodes')}
                    </button>
                </div>
            </form>
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-bold">{t('schoolCodes')}</h3>
                <ul className="space-y-3">
                    {retrievedSchools.map(school => (
                        <li key={school.id} className="p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
                            <p className="font-bold">{school.name}</p>
                            <p className="text-sm"><span className="font-semibold">{t('SchoolOwner')}:</span> <code>{school.ownerCode}</code></p>
                            <p className="text-sm"><span className="font-semibold">{t('Staff')}:</span> <code>{school.staffCode}</code></p>
                        </li>
                    ))}
                </ul>
            </div>
